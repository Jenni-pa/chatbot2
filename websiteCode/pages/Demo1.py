...
st.set_page_config(page_title="Ask Sustaino-AI", page_icon="🍃", layout="wide", initial_sidebar_state=ss.sidebar_state)
...
def main():
    ...
    with _bottom:
        col1, col2, col3,col4,col5,col6 = st.columns([40,100,100,40,60,20])
    ...
    prompt = st.chat_input("Chat input Here",disabled=ss.is_chat_input_disabled)
    with col50:
        ...
        for i, chat in enumerate(st.session_state.Chats):
            if i!=0:
                chatBoxes.append(st.container(height=300))
                with chatBoxes[i]:
                        ...
                        for message in chat:
                            with st.chat_message(message["role"]):
                                ...
                                if "assistantGraph" in message:
                                    with st.popover("📊",disabled=False,use_container_width=True):
        ...
        if prompt:
            ...
            st.session_state.Chats[j].append({"role": "user", "content": prompt})
            st.session_state.Chats[j].append({"role": "ai", "content": prompt, "assistantGraph": assistantGraph})
